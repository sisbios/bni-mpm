...
export async function GET() {
  ...
  const [totalMembers, upcomingEvents, openTasks, achievementsThisMonth] = await Promise.all([
    ...
    db.event.count({ where: { isActive: true, date: { gte: now, lte: endOfMonth } } }),
    db.weeklyTask.count({ where: { week: currentWeek, status: { in: ['pending', 'callback'] } } }),
    db.greenAchievement.count({ where: { createdAt: { gte: startOfMonth, lte: endOfMonth } } }),
  ])
  ...
  const achieverUsers = await db.user.findMany({
    where: { id: { in: topAchieversRaw.map((a) => a.userId) } },
  ...
  const rsvpStats = await db.eventRSVP.groupBy({
    by: ['status'],
    where: {
      event: { date: { gte: now, lte: endOfMonth } },
    },
    _count: { id: true },
  })
